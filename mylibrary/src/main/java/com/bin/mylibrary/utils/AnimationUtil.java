...
public class AnimationUtil {
    public static void slideview(final View view, final float p1, final float p2, final float p3, final float p4, long durationMillis, long delayMillis, final boolean startVisible, final boolean endVisible) {
        //如果处在动画阶段则不允许再次运行动画
    ...
    public static void slideButtons(Context context, final FloatingDraftButton button) {
        ...
        int radius = 7 * buttonWidth / 6;  //原来是4  调整中间时两个功能按钮距离中间按钮的距离
        int gap = 5 * buttonWidth / 4;     //原来是4  调整靠边时两个功能按钮之间的距离
        ArrayList<FloatingActionButton> buttons = button.getButtons();
        if (button.isDraftable()) {//可拖拽 展开Buttons
            showRotateAnimation(button, 0, 225); //可围成圆形
    ...
    public static void showRotateAnimation(View mView, int startDegress, int degrees) {
        ...
        float centerY = mView.getHeight() / 2.0f; //这个是设置需要旋转的角度（也是初始化），我设置的是当前需要旋转的角度
        ...
        rotateAnimation.setInterpolator(new AccelerateInterpolator()); //动画执行完毕后是否停在结束时的角度上
        rotateAnimation.setFillAfter(true); //启动动画
